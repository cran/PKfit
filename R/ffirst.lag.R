                      ...
                      MMe=FALSE) 
{
   ...
   if (MMe){
      if (is.null(Dose) || is.null(Tlag) || is.null(ka) || is.null(Vm) || is.null(Km) || is.null(Vd) ) {
      ...
      if (is.null(Dose) || is.null(Tlag) ||is.null(ka) || is.null(kel) || is.null(Vd) ) {
...
cat("--- model selection: one-compartment, extravascular PK model\n   with 1st-ordered lag-time abs./elim.")}
   ...
   for( i in 1:length(unique(PKindex$Subject)))  {
     ...
     switch(pick,
      ...
      if (MMe) {
        opt<-optim(c(par.init[3,2],par.init[4,2],par.init[5,2],par.init[6,2]),objfun, method="Nelder-Mead")
      ...
      if (MMe) {
        fm<-nlsLM(conc ~ modfun2(time, ka, Vm, Km, Vd),data=subset(PKindex,Subject==i),
            start=list(ka=opt$par[1],Vm=opt$par[2],Km=opt$par[3],Vd=opt$par[4]),
...
###        
        ...
        fm <-nlsLM(conc ~ modfun1(time, ka, kel, Vd), data=PKindex,subset=Subject==i,
             start=list(ka=opt$par[1],kel=opt$par[2],Vd=opt$par[3]),
