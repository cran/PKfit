                        ...
                        MMe=FALSE) 
{
   ...
   if (MMe){
      if(file.exists("ffirst_nolag_mm.csv")){
        par.init<-read.csv(file="ffirst_nolag_mm.csv",row.names=NULL,header=TRUE)
        ...
        repeat{
           ...
           write.csv(par.init,file="ffirst_nolag_mm.csv",row.names=FALSE,col.names=TRUE)
   } 
   else{
      if(file.exists("ffirst_nolag.csv")){
        par.init<-read.csv(file="ffirst_nolag.csv",row.names=NULL,header=TRUE)
        par.init<-edit(par.init)}
      else{
        ...
        repeat{
          ...
          write.csv(par.init,file="ffirst_nolag.csv",row.names=FALSE,col.names=TRUE)
   }
        cat("\n")
        Dose<-par.init[1,2]       
        show(par.init);cat("\n")
   ...
   if ( MMe ) {
      ...
      out <- lsoda(c(Dose, 0),c(0,time),defun,parms=c(ka=ka,kel=kel,Vd=Vd),rtol=1e-6,atol=1e-10) 
      out[-1,3] 
      }
   }
...
sink()  ### turn off temporarily to avoid logging too many warnings... -YJ
   ...
   for( i in 1:length(unique(PKindex$Subject)))  {
      ...
      if (MMe) {
        ...
        opt<-optim(c(par.init[2,2],par.init[3,2],par.init[4,2]),objfun,
                   method="Nelder-Mead",control=list(maxit=5000)) 
        nameopt<-c("ka","kel","Vd")
      ...
      if (MMe) {
        fm<-nlsLM(conc ~ modfun2(time, ka, Vm, Km, Vd), data=subset(PKindex,Subject==i),
           start=list(ka=opt$par[1],Vm=opt$par[2],Km=opt$par[3],Vd=opt$par[4]),weights=weights,
...
###        
        ...
        fm <-nlsLM(conc ~ modfun1(time, ka, kel, Vd), data=subset(PKindex,Subject==i),
             start=list(ka=opt$par[1],kel=opt$par[2],Vd=opt$par[3]),weights=weights,
