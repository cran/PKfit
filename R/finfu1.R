                   ...
                   MMe=FALSE) 
{
  ...
  if (MMe){
      if(file.exists("finfu1_mm.csv")){
        par.init<-read.csv(file="finfu1_mm.csv",row.names=NULL,header=TRUE)
        ...
        repeat{
          ...
          write.csv(par.init,file="finfu1_mm.csv",row.names=FALSE,col.names=TRUE)
  } 
  else {
      ## No MM elimination
      if(file.exists("finfu1.csv")){
        par.init<-read.csv(file="finfu1.csv",row.names=NULL,header=TRUE)
        par.init<-edit(par.init)}
      else{
        ...
        write.csv(par.init,file="finfu1.csv",row.names=FALSE,col.names=TRUE)
  }
        cat("\n")   
        Dose<-par.init[1,2]
        Tinf<-par.init[2,2]    
...
###      
     if (MMe) {
        opt<-optimx(c(par.init[3,2],par.init[4,2],par.init[5,2]),objfun,method="Nelder-Mead",control=list(maxit=5000))
        nameopt<-c("Vm","Km","Vd")
        outopt<-c(opt$p1,opt$p2,opt$p3)
     }
     else {
        opt<-optimx(c(par.init[3,2],par.init[4,2]),objfun,method="Nelder-Mead",control=list(maxit=5000))  
        nameopt<-c("kel","Vd")
...
###
        ...
        fm<-nlsLM(conc ~ modfun1(time, kel, Vd), data=subset(PKindex,Subject==i),
                start=list(kel=opt$p1,Vd=opt$p2),
                control=nls.lm.control(maxiter=500,maxfev=5000,factor=100),weights=weights,lower=c(1e-06,1e-06))   ### set 'lower=c(...)' may cause crashed.  --YJ
