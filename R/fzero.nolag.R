...
{ 
   ...
   if (MMe){
      if(file.exists("fzero_nolag_mm.csv")){
        par.init<-read.csv(file="fzero_nolag_mm.csv",row.names=NULL,header=TRUE)
        ...
        repeat{
          ...
          write.csv(par.init,file="fzero_nolag_mm.csv",row.names=FALSE,col.names=TRUE)
   } 
   else {
      ## No MM elimination
      if(file.exists("fzero_nolag.csv")){
        par.init<-read.csv(file="fzero_nolag.csv",row.names=NULL,header=TRUE)
        par.init<-edit(par.init)}
      else{
        ...
        write.csv(par.init,file="fzero_nolag.csv",row.names=FALSE,col.names=TRUE)
   }
        cat("\n")
        Dose<-par.init[1,2]       
        show(par.init);cat("\n")
