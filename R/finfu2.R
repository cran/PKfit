                  ...
                  Vd=NULL) 
{
   ...
   if(file.exists("finfu2.csv")){
        par.init<-read.csv(file="finfu2.csv",row.names=NULL,header=TRUE)
        ...
        write.csv(par.init,file="finfu2.csv",row.names=FALSE,col.names=TRUE)
        cat("\n")
        Dose<-par.init[1,2]
        Tinf<-par.init[2,2]       
        show(par.init)
...
sink()  ### turn off temporarily to avoid logging too many warnings... -YJ
   ...
   for( i in 1:length(unique(PKindex$Subject)))  {
     ...
     opt <- optim(c(par[3,2],par[4,2],par[5,2],par[6,2]),objfun,method="Nelder-Mead",control=list(maxit=5000))   
     ...
     switch(pick,                  ## show weighting scheme
       ...
       cat("1/Cp"),
       cat("1/Cp^2"));cat("\n\n")
     cat("--- model selection: two-compartment iv infusion PK model\n    with 1st-ordered elim.\n\n")
