...
if(file.exists("sbolus2_md.csv")){
   ...
   write.csv(par,file="sbolus2_md.csv",row.names=FALSE,col.names=TRUE)
   cat("\n")       
   show(par)
...
PKtime<-data.frame(time=seq(0.1,Tau*nDose,0.1))
...
if(file.exists("sbolus2.csv")){
   ...
   write.csv(par,file="sbolus2.csv",row.names=FALSE,col.names=TRUE)
   cat("\n")       
   show(par)
   ...
   if(file.exists("sim_times.csv")){
      ...
      PKtime<-data.frame(time=c(.1,.2,.3,.4,.6,.8,1,2,4,6,8,12,14,16,18,24,48,72))
      PKtime<-edit(PKtime)}
    ...
    if (pick ==1){
       ...
       if (pick ==1){
         ...
         for(i in 1:Subject)  {
           ...
           C1.lsoda<-data.frame(lsoda(c(Dose/Vd,0),c(0,time),defun,parms,atol=1e-08))}
     ...
     C1.lsoda<-list()
        for (j in 1:re){
             ...
             Vd  <- err_types(pick,par4,factor4)
               ...
               XX<-data.frame(lsoda(c(Dose/Vd,0),c(0,time1),defun,parms,atol=1e-08))}               
